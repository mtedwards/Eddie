 ...
 * @return bool
 */
function _eddie_categorized_blog() {
	// Get the categories.
	$all_the_cool_cats = get_transient( '_eddie_categories' );
	if ( false === $all_the_cool_cats ) {
		...
		set_transient( '_eddie_categories', $all_the_cool_cats );
	}
	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so _eddie_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so _eddie_categorized_blog should return false.
		return false;
	}
}
/**
 ...
 */
function _eddie_get_attachment_id_from_url( $attachment_url = '' ) {
	global $wpdb;
	$attachment_id = false;
 ...
 * @return string
 */
function _eddie_get_placeholder_image( $args = array() ) {
	$default_args = array(
		'width'            => '',
		'height'           => '',
	...
	$alt = apply_filters( '_eddie_placeholder_image_alt', esc_html__( 'Emptyhead Placeholder Image', '_s' ) );
 ...
 * }
 * _eddie_get_placeholder_unsplash();
 * @return string
 */
...
/* Theatre Unsplash for use in my sites featured-images-post-thumbnails
*
*/
function _ed_unsplash($width, $height) {
  echo _eddie_get_placeholder_unsplash(array(
    "width"    => $width,
    "height"   => $height,
    "category" => "people",
    "keywords" => ""
