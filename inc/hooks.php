 ...
 * @return array
 */
function _eddie_body_classes( $classes ) {
...
/**
 * Flush out the transients used in _eddie_categorized_blog.
 */
function _eddie_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( '_eddie_categories' );
}
add_action( 'delete_category', '_eddie_category_transient_flusher' );
add_action( 'save_post',     '_eddie_category_transient_flusher' );

/**
 * Customize "Read More" string on <!-- more --> with the_content();
 */
function _eddie_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', '_s' ) . '...</a>';
}
add_filter( 'the_content_more_link', '_eddie_content_more_link' );
 ...
 */
function _eddie_custom_mime_types( $mimes ) {
	$mimes['svg'] = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', '_eddie_custom_mime_types' );
...
add_shortcode( 'svg', '_eddie_display_svg' );
